...
public struct MainView: View {
  ...
  public var body: some View {
    ZStack {
      ...
      if mainObservable.currentPresentationDetent == .height(
        ...
        FieldCarouselButton(
          ...
          theme: Theme(
            rawValue: observable
              .lineup[mainObservable.currentIndex]
              .selectedTheme
          ) ?? Theme.blueGray )
      }
    }
  ...
  private func buttonView(isLeft: Bool) -> some View {
    Button(action: {
      if isLeft {
        ...
        mainObservable.currentIndex = min(mainObservable.currentIndex + 1, 3)
      }
    }, label: {
      Image(systemName: isLeft ? "chevron.left" : "chevron.right")
        ...
        .font(.system(size: 20))
        .opacity(isLeft ?
                 (mainObservable.currentIndex == 0 ? 0.3 : 1) : (mainObservable.currentIndex == 3 ? 0.3 : 1))
    })
    .padding()
            ...
            VStack {
              Image(systemName: "flag.2.crossed")
                ...
                .foregroundColor(
                  ...
                  Color.black :
                    Theme(
                      rawValue: lineup.selectedTheme
                    )?.textColor ?? Theme.blueGray.textColor
                )
...
struct TeamInfo: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      HStack {
        Text(observable.lineup[mainObservable.currentIndex].lineupName)
          ...
          .foregroundStyle(
            Theme(
              rawValue: observable.lineup[mainObservable.currentIndex]
                .selectedTheme
            )?.textColor ?? Theme.blueGray.textColor
          )
        if mainObservable.currentPresentationDetent == .height(CGFloat.editHeight) {
    ...
    .padding(
      .leading,
      mainObservable.currentPresentationDetent == .height(
        CGFloat.editHeight
      ) ? 24 : 0
    )
    .blur(
      radius: (
        mainObservable.isSharing || mainObservable.isShowTeamSheet
      ) ? 10 : 0
    )
    .animation(
      .easeInOut(
      ...
      value: mainObservable.currentPresentationDetent
    )
  }
}
